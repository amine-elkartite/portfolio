...
async function invalidateMessageData(){await Promise.all([queryClient.invalidateQueries({queryKey:['messages']}),queryClient.invalidateQueries({queryKey:['messages','unread-count']}),queryClient.invalidateQueries({queryKey:['dashboard']})]);}

export default function MessageDetail(){
  const {authenticated,locked}=useAuth();
  const {colors}=useTheme();
  const {id:rawId}=useLocalSearchParams<{id:string}>();const id=Number(rawId);
  const [actionError,setActionError]=useState('');const [copied,setCopied]=useState(false);
  const query=useQuery({queryKey:['messages',id],queryFn:()=>messagesApi.one(id),enabled:authenticated&&Number.isFinite(id)&&id>0});
  const statusMutation=useMutation({mutationFn:(status:Message['status'])=>messagesApi.setStatus(id,status),onSuccess:invalidateMessageData,onError:error=>setActionError(error instanceof Error?error.message:'Unable to update message.')});
  useEffect(()=>{if(query.data?.status!=='unread')return;let active=true;void messagesApi.setStatus(id,'read').then(()=>active?invalidateMessageData():undefined).catch(()=>undefined);return()=>{active=false;};},[id,query.data?.status]);
  ...
  const remove=()=>Alert.alert('Delete message?',`Delete the message from ${message.name}?`,[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:async()=>{try{await messagesApi.remove(id);await invalidateMessageData();router.back();}catch(error){setActionError(error instanceof Error?error.message:'Unable to delete message.');}}}]);
  const copyEmail=async()=>{await Clipboard.setStringAsync(message.email);setCopied(true);setTimeout(()=>setCopied(false),1600);};
  return <Screen>
