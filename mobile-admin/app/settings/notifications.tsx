...
export default function NotificationSettings(){
  const {authenticated,locked}=useAuth();const {colors}=useTheme();const {permission,refreshPermission,openDeviceSettingsHint}=useNotifications();
  const query=useQuery({queryKey:['notification-settings'],queryFn:authApi.notificationSettings,enabled:authenticated});
  const [prefs,setPrefs]=useState<NotificationPreferences|null>(null);const [error,setError]=useState('');const [saving,setSaving]=useState(false);
  useEffect(()=>{if(query.data)setPrefs(query.data);},[query.data]);
  if(locked)return <Redirect href="/unlock"/>;if(!authenticated)return <Redirect href="/login"/>;
  if(query.isLoading||!prefs&&!query.isError)return <Screen><LoadingState/></Screen>;
  if(query.isError||!prefs)return <Screen><ErrorState message={query.error instanceof Error?query.error.message:'Unable to load notification settings.'} onRetry={()=>void query.refetch()}/></Screen>;
  const save=async(key:keyof Omit<NotificationPreferences,'pushRegistered'>,value:boolean)=>{const next={...prefs,[key]:value};setPrefs(next);setSaving(true);setError('');try{const updated=await authApi.updateNotificationSettings({[key]:value});setPrefs(updated);}catch(e){setPrefs(prefs);setError(e instanceof Error?e.message:'Unable to save notification preference.');}finally{setSaving(false);}};
