...
function initialState(resource:ManagedResource,row?:Row):FormState{
  const state=Object.fromEntries(resourceConfigs[resource].fields.map(field=>[field.key,field.kind==='boolean'?true:field.kind==='choice'?(field.choices?.[0]?.value||''):field.kind==='number'&&field.required?'0':''])) as FormState;
  if(row){for(const field of resourceConfigs[resource].fields){const value=row[field.key];state[field.key]=field.kind==='boolean'?Boolean(value):value==null?'':String(value);}}
  return state;
}

function ResourceForm({resource,id,row}:{resource:ManagedResource;id:'new'|number;row?:Row}){
  const {colors}=useTheme();
  const config=resourceConfigs[resource];
  const editing=id!=='new';
  const [values,setValues]=useState<FormState>(()=>initialState(resource,row));
...
export function ResourceEditor({resource,id}:{resource:ManagedResource;id:'new'|number}){
  ...
  return <ResourceForm resource={resource} id={id} row={query.data}/>;
}
