...
export function ErrorState({message,onRetry}:{message:string;onRetry?:()=>void}){const {colors}=useTheme();return <Card><Text style={{fontWeight:'700',fontSize:16,marginBottom:8,color:colors.text}}>Something went wrong</Text><Text style={{marginBottom:12,color:colors.textMuted}}>{message}</Text>{onRetry?<PrimaryButton title="Try Again" onPress={onRetry}/>:null}</Card>;}
