...
export async function logout(req,res) {
  const token=req.headers.authorization?.startsWith('Bearer ')
    ...
    : (req.headers.cookie||'').split(';').map(value=>value.trim()).find(value=>value.startsWith('portfolio_session='))?.slice('portfolio_session='.length);

  if(token&&process.env.JWT_SECRET){
    try{
      const payload=jwt.verify(decodeURIComponent(token),process.env.JWT_SECRET,{algorithms:['HS256'],issuer:'amine-portfolio',audience:'portfolio-admin'});
      await pool.execute('UPDATE users SET token_version=token_version+1 WHERE id=?',[payload.sub]).catch(error=>console.warn('[logout revoke skipped]',error.code||error.name));
    }catch(error){
      if(!['JsonWebTokenError','TokenExpiredError'].includes(error.name)) console.warn('[logout token]',error.code||error.name);
    }
  }

  res.clearCookie('portfolio_session',{...cookieOptions,maxAge:undefined});
...
export async function me(req,res) {
  ...
  try{
    ...
    avatar=rows[0]?.setting_value||null;
  }catch(error){
    console.warn('[auth me avatar unavailable]',error.code||error.name);
  }
  return ok(res,{...user,avatar});
}
